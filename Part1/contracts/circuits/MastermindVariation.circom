...
template MastermindVariation() {
    ...
    var j = 0;
    var k = 0;
    component lessEqThan[10];
    component equalGuess[10];
    component equalSoln[10];
    var equalIdx = 0;

    // Create a constraint that the solution and guess digits are all less than or equals 8
    for (j=0; j<5; j++) {
        lessEqThan[j] = LessEqThan(4);
        lessEqThan[j].in[0] <== guess[j];
        lessEqThan[j].in[1] <== 8;
        lessEqThan[j].out === 1;
        lessEqThan[j+5] = LessEqThan(4);
        lessEqThan[j+5].in[0] <== soln[j];
        lessEqThan[j+5].in[1] <== 8;
        lessEqThan[j+5].out === 1;
        for (k=j+1; k<5; k++) {
